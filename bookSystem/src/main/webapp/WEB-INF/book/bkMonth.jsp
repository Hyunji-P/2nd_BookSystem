...
<div class="container">
  ...
  <hr>         
  <%-- [검색 모드] ==== 시작 ==== --%>
	<form id="contact-form search" method="get" action="${contextPath}/bkMonthStat.bk" role="form">
		<div id="search">
			<select class="form-control" name="mode" id="mode"
				data-toggle="tooltip" title="검색할 조건을 선택하세요!">
				<option class="form-control" value="all">전체</option>
				<option class="form-control" value="reBookNumber">도서번호</option>
			</select> 
    ...
    <tbody>
    	...
    	<c:forEach var="item" items="${requestScope.resultList}">
    		<tr>
    			...
    			<c:forEach var="mapData" items="${item.bookMap}">
    				...
    				<c:if test="${mapData.value != 0}">
	    				<td>
	    					<a href="${contextPath}/bkMonthDetail.bk?month=${mapData.key}&bookNumber=${item.bookNumber}">
	    						${mapData.value}
	    					</a>
	    				</td>
    				</c:if>
